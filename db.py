...
def save_call_log(phone: str, duration: int, transcript: str, summary: str = "", recording_url: str = "", caller_name: str = "") -> dict:
    ...
    if not url or not key:
        ...
        try:
            ...
            import csv
            # Modified to use the local main project's data files
            log_path = r"d:\university_counselor\data\conversation_logs.csv"
            os.makedirs(os.path.dirname(log_path), exist_ok=True)
            file_exists = os.path.exists(log_path)
            with open(log_path, mode="a", newline="", encoding="utf-8") as f:
                writer = csv.writer(f)
                if not file_exists:
                    writer.writerow(["Date", "Phone", "Duration", "Summary", "Transcript"])
                writer.writerow([datetime.now().strftime("%Y-%m-%d %H:%M:%S"), phone, duration, summary, transcript])
        except Exception as e:
            logger.error(f"Failed to log locally: {e}")
        return {"success": False, "message": "Supabase not configured, fell back to CSV."}
